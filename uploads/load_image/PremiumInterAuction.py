...
downloads_dir = os.path.join(os.path.dirname(__file__), '../../public', 'downloads', 'Premium_Inter_auction')
output_dir = os.path.join(os.path.dirname(__file__), '../../public', 'downloads', 'Premium_Inter_auction')
log_dir = os.path.join(os.path.dirname(__file__), '../../logs', 'Premium_Inter_auction')
...
def process_csv(file_path, id):
    ...
    for _, row in df.iterrows():
        ...
        # Combine licensePlate and province to form the folder name
        reg_folder = f"{iden}-{license_plate}-{province}".strip()
        iden += 1

        # Skip rows where either licensePlate or province is empty or null
        if not license_plate or license_plate.lower() == 'nan' or not province or province.lower() == 'nan':
            continue

        reg_image_dir = os.path.join(image_dir, reg_folder)
        create_dir(reg_image_dir)

        image_urls = str(row['images']).split(' | ') if pd.notna(row['images']) else []
        new_url = ''
        stop = False
        ...
        for url in image_urls:
            ...
            if status_code == 200:
                new_url = os.path.join('downloads', 'Premium_Inter_auction', base_folder_name, 'images', reg_folder).replace('\\', '/')
                successful_images += 1
