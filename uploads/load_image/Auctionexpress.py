...
downloads_dir = os.path.join(os.path.dirname(__file__), '../../public', 'downloads', 'Auction_express')
output_dir = os.path.join(os.path.dirname(__file__), '../../public', 'downloads', 'Auction_express')
log_dir = os.path.join(os.path.dirname(__file__), '../../logs', 'Auction_express')
...
def process_csv(file_path, id):
    ...
    for _, row in df.iterrows():
        car_reg_no = f"{iden}-{str(row['car_reg_no']).strip()}"
        iden += 1

        if not car_reg_no or car_reg_no.lower() == 'nan':
            continue

        reg_image_dir = os.path.join(image_dir, car_reg_no)
        create_dir(reg_image_dir)

        image_urls = str(row['Imagepath']).split(' | ') if pd.notna(row['Imagepath']) else []
        new_url = ''
        stop = False
        ...
        for url in image_urls:
            ...
            if status_code == 200:
                new_url = os.path.join('downloads', 'Auction_express', base_folder_name, 'images', car_reg_no).replace('\\', '/')
                successful_images+=1


        row['Imagepath'] = new_url
