        ...
        const readCSV = () => {
            return new Promise((resolve, reject) => {
                fs.createReadStream(csvFilePath)
                    ...
                    .pipe(csv())
                    .on('data', (row) => {
                        const sellerGroupValue = reverseVendorMap.get(`${[row.SELLER_GROUP]}`);
                        const data = {
                            Company: auctionName,
                            Auction_date: auctionDate,
                            Auction_name: '',
                            Auction_location: row.stage_location || '',
                            Auction_lane: row.AUCTION_STAGE|| '', 
                            Order_No: row.PRODUCT_NO || '',
                            Remark1: row.PRODUCT_NOTE || '',
                            Remark2: row.note_txt ||'',
                            Reserve_price: 0.0,
                            Start_price: parseFloat((row.PRICE_START || '0'))|| 0.0,
                            Category: row.PRODUCT_CATEGORY_SUB || '',
                            Reg_No: row.PRODUCT_REGISTRATION || '',
                            Reg_Province: row.PRODUCT_PROVINCE||'',
                            Brand: row.PRODUCT_BRAND || '',
                            Model: row.PRODUCT_GENERATION || '',
                            Engine_displacement: parseInt(row.PRODUCT_CC) ||'',
                            Gear: row.PRODUCT_GEAR || '',
                            Fuel:  '',
                            Color: row.PRODUCT_COLOR ||'',
                            Car_man_year: null,
                            Car_reg_year: parseInt(row.PRODUCT_YEAR) || 0,
                            Mile: parseInt((row.Mileage || '0')) || 0,
                            Engine_No: row.PRODUCT_NO_ENGINE || '',
                            Chassis_No: row.PRODUCT_NO_CHASSIS|| '',
                            Grade_overall: row.PRODUCT_GRADE || '',
                            Grade_frame:   '',
                            Grade_Internal: '',
                            Seller_name: sellerGroupValue ? sellerGroupValue.join(', ') : '',
                            Seller_code: row.SELLER_GROUP || '',
                            Sourcing_type: '',
                            Car_tax_expired_date: row.PRODUCT_DATE_TAX? moment((row.PRODUCT_DATE_TAX).replace(/^(\d{4})/, (match, p1) => p1 > 2500 ? p1 - 543 : p1), 'YYYY-MM-DD').format('YYYY-MM-DD') : null,
                            Car_title_group: '',
                            imageUrl: `${row['NF_PRODUCT_PICTURE']} | ${row.link}` || '',
                            csv: fileName
                            };
                        results.push(data);
                    })
